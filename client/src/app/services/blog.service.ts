...
export class BlogService {
  ...
  postComment(id, comment) {
    ...
    public confirm(title: string, message: string): Observable<boolean> {
     ...
     createSearchOptions(pageSize){
    let params: URLSearchParams = new URLSearchParams();
    params.set('pageSize',pageSize);
    this.options.search = params;
    }

    getSinglePageBlogs(page:number,singlePageSize){
     this.createAuthenticationHeaders();
     this.createSearchOptions(singlePageSize);
     return this.http.get(this.domain + 'blogs/paged/'+page, this.options).map(res => res.json())
     .catch((error:any) => Observable.throw(error.json().error));
    }
    getBlogsOfCurrentUser(){
      this.createAuthenticationHeaders();
      return this.http.get(this.domain+'blogs/blogsOfUser',this.options).map(res=>res.json()).catch((Error:any)=>
