...
var storage = multer.diskStorage({ //multers disk storage settings
  ...
  filename: function (req, file, cb) {
    ...
    cb(null, req.decoded.userId + '.' + file.originalname.split('.')[file.originalname.split('.').length - 1]);
  }
});
...
module.exports = (router) => {
  ...
  router.post('/comment', (req, res) => {
    ...
    /* ===============================================================
     Route to get paged blogs
  =============================================================== */
  router.get('/paged/:pageNumber', (req, res) => {
    if (!req.params.pageNumber) {
      res.json({ success: false, message: 'No page was provided' })
    } else {
      const singlePageSize = parseInt(req.query.pageSize, 10);
      Blog.find({}).skip((req.params.pageNumber - 1) * singlePageSize).limit(singlePageSize).sort({ '_id': -1 }).exec(function (err, articles) {
        if (err) {
          res.json({ success: false, message: err });
        } else {
          Blog.count({}, function (err, count) {
            res.json({ success: true, data: articles, totalCount: count });
          })
  ...
  router.post('/avatar', function (req, res) {
    upload(req, res, function (err) {
      ...
      res.json({ success: true, data:"Image was succesfully updated"});
    });
  });
  /* ===============================================================
     Route to get (saved) user avatar
  =============================================================== */
  router.get('/avatar', function (req, res) {
    ...
    var in_this_counter = 0;
    var finded = false;

    fs.readdir(__dirname + '/../uploads/', (err, files) => {
      files.forEach(file => {

        in_this_counter = in_this_counter + 1;
        if (file.indexOf(req.decoded.userId) > -1) {
          finded = true;
          res.sendFile(file, {
        ...
        //sending default user avatar
        if (in_this_counter == number_of_files && finded == false) {
  ...
  /* ===============================================================
    ...
    =============================================================== */
  router.get('/blogsOfUser', function (req, res) {
    User.findOne({ _id: req.decoded.userId }).select('username').exec((err, user) => {
      if (err) {
        res.json({ success: false, message: "You are not logged in" })
      } else {
        Blog.find({ createdBy: user.username }).exec((err, blogs) => {
          if (err) {
            res.json({ success: false, message: 'You should create some blogs' })
          } else {
